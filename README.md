...
- 🔥 **生产力热力图**：近 30 天每日完成数 GitHub 风格色阶图（纯 CSS Grid，星期横排 + 日期纵排）
...
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| 主题与外观、壁纸、标签、快捷导航、纪念日倒计时、发薪日       | **微信推送 UID** 等用户级凭证——进数据库等于多一份泄露面，换设备重填一次即可          |
...
| **CSP 与真实调用域名一致**        | `connect-src` 覆盖高德 `restapi.amap.com`、`*.supabase.co`、`wxpusher.zjiecode.com`（当时的 BYOK 还直连 `api.deepseek.com`、`open.bigmodel.cn`——**BYOK 撤除后不再调用**，见第 1262 行的开放策略）；`img-src https:` 覆盖 Google favicon 服务与 GitHub 头像                          |
...
- **未同步的两类数据**（有意为之，见第九阶段）：微信推送 UID 等用户级凭证与设备相关项留在本机；
